...
export class Transaction {
    public get hash(): string {
        return u.reverseHex(u.sha256(this.serielize()));
    }
    ...
    public addNEP5(data: string) {
        ...
        this.attributes.push({
            ...
            data: u.reverseHex(u.str2hexstring('from iwallic at ' + new Date().getTime()))
        });
    }
    ...
    private serielizeOutput(): string {
        ...
        for (const output of this.vout) {
            const value = u.num2fixed8(typeof output.value === 'string' ? parseInt(output.value, 0) : output.value);
